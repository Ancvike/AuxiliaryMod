...
public class UIMovement extends Function {
    ...
    public Table setDialogTable_mobile() {
        return new Table(t -> {
            ...
            t.add("X:").color(Color.yellow).size(10f);
            xText = t.field(null, text -> xText()).get();
            t.row();
            t.add("Y:").color(Color.yellow).size(10f);
            yText = t.field(null, text -> yText()).get();
            t.row();
            t.button("确定", this::click_yes).size(120f, 50f).row();
            t.button("取消", this::click_no).size(120f, 50f).row();
            t.button("重置", this::click_reset).size(120f, 50f).row();
        });
    }
